...
const InstagramPost = ({ item }) => {
    ...
    const renderComment = ({ item }) => {
        ...
        // try {
        //     const response = await axios.get(`http://192.168.31.161:8000/profile/${item.sender_id}`);
        //     const userData = response.data;
        //     setUsername(userData.username);
        //     setUserImageUrl(userData.profilePicture);
        // } catch (error) {
        ...
        return (
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginVertical: 10 }}>
                <Image source={{ uri: userImageUrl || 'https://example.com/placeholder.png' }} style={{ width: 40, height: 40, borderRadius: 20, marginRight: 10 }} />
                <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', gap: 5 }}>
                        <Text style={{ fontWeight: 'bold', flex: 3, fontSize: 12 }}>{item.username || item.sender_id}</Text>
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={{ color: "white", marginLeft: 10, marginBottom: 1 }}>{username} : {description}</Text>
            <TouchableOpacity onPress={toggleModal}>
